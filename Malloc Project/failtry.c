...
static const size_t chunksize = (1 << 10);    // requires (chunksize % 16 == 0) (mark)
...
{   
    ...
    //update the next block's prev_allocated states
...
static block_t *find_fit(size_t asize)
{
    ...
    int fits = 0;
    if(asize <= s_size){
        for (block = list_16_root; (block != NULL) && (fits < n_fits); block = block->next)
        {
    ...
    if(asize <= m_size){
        for (block = list_32_root; (block != NULL) && (fits < n_fits); block = block->next)
        {
            if (!(get_alloc(block)) && (asize <= get_size(block)))
            {
                fits++;
    ...
    if(asize <= l_size){
        for (block = list_64_root; (block != NULL) && (fits < n_fits); block = block->next)
        {
            if (!(get_alloc(block)) && (asize <= get_size(block)))
            {
                fits++;
    ...
    for (block = list_large_root; (block != NULL) && (fits < n_fits); block = block->next)
    {
        if (!(get_alloc(block)) && (asize <= get_size(block)))
            {
                fits++;
